...
foodBoT <- foodBoT %>%
  ...
  summarize(Quantity = sum(Y2019, na.rm=TRUE))
...
foodBoTClean[foodBoTClean == "Congo"] <- "Democratic Republic of the Congo"
...
foodBoTClean[foodBoTClean == "Netherlands Antilles (former)"] <- "Curaçao"
...
# making labels
labels <- sprintf("<strong>%s</strong><br/>%g thousand tonnes", borders$ADMIN, borders$`Balance of Trade`) %>%
