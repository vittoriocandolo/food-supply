...
Riferimenti all'economia potranno essere talvolta presenti, così come sono presenti riferimenti a condizioni geografiche o sociali che hanno effetti più o meno diretti sui dati trattati.
...
La maggior parte sono stati ottenuti tramite l'opzione di bulk download offerta dal sito FAOSTAT e solo in alcuni casi tramite interrogazioni poste al sistema stesso per motivi di formati e compatibilità.  
I dataset 1 e 8 sono invece accessori, necessari per alcune visualizzazioni.
...
sUAChinaC <- filter(sUAChinaC,!(grepl('Total', item))) %>%
  ...
  rename(
    ...
    foodSupply = `Food supply quantity (tonnes)`,
    loss = Loss,
    processed = Processed,
    feed = Feed)
...
# computing totals
cCTot = sum(sUAChinaC$consumption, na.rm = TRUE)
iCTot = sum(sUAIndiaC$consumption, na.rm = TRUE)
...
Possiamo quindi concludere che la differenza evidenziata dalla mappa è da ricondurre ad un maggiore consumo interno in Cina.
...
Il BoT è però un dato complesso per cui vale la pena di analizzare tramite scatter plot queste correlazioni.
...
Nelle righe successive della tabella compaiono gli altri stati del continente come contesto.  
Area espressa in migliaia di ettari.
...
Questo grafico mostra l'area in ettari dedicata in Canada e in USA alle prime 5 produzioni di ciascuno stato
...
export <- read_csv("./data/Trade_CropsLivestock_E_All_Data.csv") %>%
  ...
  rename(
    country = Area,
    export1000Tonnes = `Export Quantity_tonnes`,
...
# to fit better in the kable
export[export == "United States of America"] <- "USA"
...
Come sempre iniziamo stabilendo qualche punto per poi mostrare dei modelli.

I dati per questa parte provengono da dataset usati in precedenza e da uno nuovo.  
...
Come per la mappa precedente è stata effettuata una normalizzazione sui valori.
...
## Mappa mondiale dell'export brasiliano

L'export verso la Cina è 8 volte superiore a quello verso il secondo paese partner.
...
## Mappa mondiale dell'export brasiliano II

Anche in questo caso c'è un outlier relativo, l'Iran.
...
## Mappa mondiale dell'export brasiliano III

É necessaria questa terza visualizzazione per poter valutare gli altri partner.
...
pstc <- read_csv("./data/Inputs_Pesticides_Use_E_All_Data.csv") %>%
  ...
  # making ready to be displayed as a table
...
# shorter names for the table
pstc[pstc == "Russian Federation"] <- "Russia"
pstc[pstc == "United States of America"] <- "USA"

# df for production - pesticides usage correlation
...
# making ready to be displayed as table
sNup <- group_by(sNu, country) %>%
  summarize(quantity = sum(synthetic, manure, deposition, cropRemoval, bioFixation, na.rm = TRUE)) %>%
  # ungrouping
  ungroup()

# df for production - nutrients usage correlation
sNuP <- inner_join(sNup, production)

# building bar plot for pesticides usage (top 10 countries only)
barPstc <- ggplot(data = pstc[1:10, ]) +
  ...
  labs(
    ...
    subtitle = "Primi 10 paesi per uso",
    caption = "Dati 2020",
    x = "",
    y = "Pesticide Usage (MT)",
    fill = "Country" # not working
  )

# building production - pesticides correlation scatter plot
scatterPstcP <- ggplot(pstcP, aes(production, pesticides)) +
  ...
  labs(
    ...
    caption = "Dati 2020",
    x = "Production (K MT)",
    y = "Pesticides Usage (MT)"
  )

# building bar plot for nutrients per ha usage (top 7)
barSNupHa <- ggplot(data = sNupHa[1:7, ]) +
  ...
  labs(
    ...
    caption = "Dati 2020",
    x = "",
    y = "Nutrient Usage (MT per ha)",
    fill = "Country" # not working
  )

# building production - nutrients correlation scatter plot
...
boxProdEt <- ggplot(data = eTot[1:92,]) + # if I keep every row it collapses, this way every state has a fair share
...
eTime <- read_csv("./data/Emissions_Totals_Time_E_All_Data.csv.csv") %>%
  rename(country = Area)
...
eTime <- select(eTime, country, Element, Year, Value) %>%
  ...
  summarize(Value = sum(Value, na.rm = TRUE)) %>%
  # Too many rows for plotly()
  # So I have to choose a partition
  # To do so I have to order the df in a way so that countries entries are grouped
  # and I have to make sure I don't lose countries with interesting values
  # So we do pivot_wider, order and pivot_longer back
...
# building animated scatter plot
...
# actual building
scatterETime <- plot_ly( # movements on x axis are due to NA values...
  data = eTime[1:2950, ], # biggest number that works on my machine, 50 countries (2950 / 59)
  x = ~country,
  y = ~`value`,
  frame = ~year,
  color = ~country,
...
Questo grafico mette in mostra le differenti emissioni dei maggiori produttori.
